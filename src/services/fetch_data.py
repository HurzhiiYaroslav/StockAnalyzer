...
def fetch_single_ticker_history(ticker_symbol: str, start_date: str, end_date: str, interval: str = "1d"):
    ...
    try:
        ...
        if not data_to_return.empty:
            data_to_return.index = pd.to_datetime(data_to_return.index).normalize()
...
def download_historical_data_for_tickers(tickers: list, start_date: str, end_date: str) -> dict:
    ...
    for i, ticker_symbol in enumerate(tickers):
        df = fetch_single_ticker_history(
        ...
        if not df.empty:
            print(f"({i+1}/{total_tickers}) Fetched data for {ticker_symbol}... OK")
            all_data[ticker_symbol] = df
        else:
            print(f"({i+1}/{total_tickers}) Fetching data for {ticker_symbol}... FAILED.")
            
    print(f"--- Download complete. Successfully fetched data for {len(all_data)} out of {total_tickers} tickers. ---")
    return all_data
...
def get_fundamental_data(ticker_symbol: str, period: str = "Q") -> bool:
    ...
    params = {
        "apikey": config.DATAJOCKEY_API_KEY,
        "filetype": "json",
        "period": period,
    ...
    try:
        ...
        if response.status_code == 429:
            print(f"    ERROR: Rate limit hit (429) for {ticker_symbol}. Stopping to avoid further issues.")
            return False
...
def get_price_history_for_chart(ticker_symbol: str, days_back: int = 365):
    try:
        ...
        if not hist.empty:
            hist_processed = hist[['Close']].copy()
            hist_processed.dropna(inplace=True)
